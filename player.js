const PLAYER_JS_T = {
    ...
    playerClosed: '播放器已关闭',
    playerOpenPlayingXxx: title => '播放器已打开，正在播放《{title}》'.replace('{title}', title),
    playerOpenWithXxx: title => '播放器已打开，显示《{title}》'.replace('{title}', title),
    unmute: '取消静音',
    volume: '音量',
    xxxHours: hours => '{xxx} 小时'.replace('{xxx}', hours),
    xxxMinutes: minutes => '{xxx} 分钟'.replace('{xxx}', minutes),
    xxxSeconds: seconds => '{xxx} 秒'.replace('{xxx}', seconds)
};
